...
class spike_parallel(pluginTemplate):
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        #      not please change appropriately      #-march={0}
        self.compile_cmd = 'riscv32-unknown-elf-gcc  \
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            if cgf_file is not None:
                ...
                execute += self.dut_exe + '  --isa=rv32imc +signature={1} +signature-granularity=4 {2};'.format(self.isa, sig_file, elf)
            ...
            if cgf_file is not None:
                coverage_cmd = 'riscv_isac --verbose error coverage -d \
                        ...
                        -e ref.elf -c ' + ' -c '.join(cgf_file) + cov_str + " -x "+ self.xlen + " -f "+ self.flen+';'
            else:
                coverage_cmd = 'riscv_isac --verbose error coverage -d \
                        --parser-name spike -o coverage.rpt  \
                        --sig-label begin_signature  end_signature \
                        --test-label rvtest_code_begin rvtest_code_end \
                        -e dut.elf -c ' + cov_str + " -x "+ self.xlen + " -f "+ self.flen+';'
